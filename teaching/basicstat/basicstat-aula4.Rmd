...
\begin{center}
$S=\sqrt{\frac{\overset{n}{\underset{i=1}{\sum}}f_i.(Xi-\overline{X})^{2}}{\overset{n}{\underset{i=1}{\sum}}f_{i-1}}}$
\end{center}
...
- A propriedade 1 continua válida para a variância, mas a propriedade 2 se altera, pois se multiplicarmos todos os valores por uma constante (diferente de zero), a variância será multiplicada por essa mesma constante elevada ao quadrado
...
- $CAP > 1 \Rightarrow$ Assimetria positiva (ou à direita) 

### Medidas de assimetria

Utilizamos a função `skewness` do pacote `e1071` para calcular o coeficiente de assimetria:

```{r assimetria, eval=TRUE, echo=TRUE, comment=NA}
library(e1071)
...
- O n-ésimo momento de uma distribuição é definido como $E[X^n]$
